...
class OrientationModel(Model):
    def __init__(self,
        ...
        """
        A model simulating cortical activity in the human visual system

        :param parameters: parameters information used by the model
        :param channels: number of tuning curves or channels
        :param input_dim: input dimension of the model
        :param input_activity: input image (if this is None, it must be set in the model.update() method).
...
def tuning_function(channels: int) -> Callable[[Union[List, float], float], Union[List, float]]:
    """
    Given a number of channels, this function returns a tuning function used by the orientation model.
    ...
    reduce the disturbances in the model since these tiny perturbations will amplify as the model updates.
    :param channels: number of orientation channels used by the model
    :return: A tuning function used by the model
    """
    kappa = channels
    max_diff = 10
    x = np.arange(0, 180)
