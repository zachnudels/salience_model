...
class Model:
    ...
    The stages of the salience_model are implemented as 3D lists where (0,1,2) corresponds to
    ...
    def __init__(
        ...
        # Construct the salience_model nodes with one activity map for each feature (preference)
        ...
        self.V1_X, self.V1_Y = np.linspace(-1, 1, V1_dim[1]), np.linspace(
            -1, 1, V1_dim[0]
        )
        self.V2_X, self.V2_Y = np.linspace(-1, 1, V2_dim[1]), np.linspace(
            -1, 1, V2_dim[0]
        )
        self.V4_X, self.V4_Y = np.linspace(-1, 1, V4_dim[1]), np.linspace(
            -1, 1, V4_dim[0]
        )
        ...
        self.recordings = {
            layer: {
                site: deepcopy(initial_recordings[site])
                for site in recording_sites[layer]
            }
    ...
    def update(
        self, timestep: float, bu_input: np.ndarray = None, td_input: np.ndarray = None
    ):
        ...
        V2_feedforward = feedforward_signal(
            self.V1.V, self.V2.ff_kernel, self.V1_X, self.V1_Y, self.V2_X, self.V2_Y
        )
        self.V2.update(V2_feedforward, timestep)

        V4_feedforward = feedforward_signal(
            self.V2.V, self.V4.ff_kernel, self.V2_X, self.V2_Y, self.V4_X, self.V4_Y
        )
        ...
        V1_2_feedback = feedback_interp(
            self.V2.W, self.V1_X, self.V1_Y, self.V2_X, self.V2_Y
        )
        V1_4_feedback = feedback_interp(
            self.V4.W, self.V1_X, self.V1_Y, self.V4_X, self.V4_Y
        )
        V1_feedback = np.zeros_like(self.V1.V)
        for i in range(len(self.features)):
            V1_feedback[:, :, i] = ndimage.correlate(
                V1_2_feedback[:, :, i] + V1_4_feedback[:, :, i],
                self.V1.fb_kernel,
                mode="nearest",
            )
        self.V1.update_D(V1_feedback, timestep)

        _V2_feedback = feedback_interp(
            self.V4.W, self.V2_X, self.V2_Y, self.V4_X, self.V4_Y
        )
        V2_feedback = np.zeros_like(self.V2.W)
        for i in range(len(self.features)):
            V2_feedback[:, :, i] = ndimage.correlate(
                _V2_feedback[:, :, i], self.V2.fb_kernel, mode="nearest"
            )
        ...
        for layer in self.recording_sites:
            ...
            for site in self.recording_sites[layer]:
                self.recordings[layer][site].append(
                    mean_activity[self.recording_sites[layer][site]]
                )
...
def feedforward_signal(
    lower_activity_map: np.ndarray,
    kernel: np.ndarray,
    lower_X: np.ndarray,
    lower_Y: np.ndarray,
    higher_X: np.ndarray,
    ...
    """
    higher_activity = np.zeros(
        (higher_X.shape[0], higher_Y.shape[0], lower_activity_map.shape[2])
    )
    for i in range(lower_activity_map.shape[2]):
        activity = ndimage.correlate(
            lower_activity_map[:, :, i], kernel, mode="nearest"
        )
        f = RectBivariateSpline(lower_X, lower_Y, activity)
        higher_activity[:, :, i] = f(higher_X, higher_Y)
    return higher_activity


def feedback_interp(
    higher_activity_map: np.ndarray,
    lower_X: np.ndarray,
    lower_Y: np.ndarray,
    higher_X: np.ndarray,
    higher_Y: np.ndarray,
    ...
    """
    lower_activity = np.zeros(
        (lower_X.shape[0], lower_Y.shape[0], higher_activity_map.shape[2])
    )
    for i in range(higher_activity_map.shape[2]):
        f = RectBivariateSpline(higher_X, higher_Y, higher_activity_map[:, :, i])
